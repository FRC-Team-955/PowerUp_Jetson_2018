...
int main () {
	...
	while (true) {
		...
		//cv::Point2f mid_point (1000.0, 2000.0);
		//cv::Point2f end_point (1000.0, 5000.0);

		/*
		auto func = SplineWrap (
						{ start_point, 0.25, 1.0, MiscMath::pi / 2.0f, wheel_width, false },
						{ FieldDimension::Switch::front_center_left - cv::Point2f(0.0, wheel_width / 2.0f), 0.25, 1.0, MiscMath::pi / 2.0f, wheel_width, false }
						);
						*/

		CircleWrap func2 (cv::Point2f(5000.0, 5000.0), 2000.0, 2000.0);
		SpiralWrap func (cv::Point2f(5000.0, 5000.0), 100.0, 10.0 * MiscMath::pi);
		full_path.push_back(TankDriveCalculator (func, 660.0 / 2.0, 20.0, false));	
		full_path.push_back(TankDriveCalculator (func2, 660.0 / 2.0, 20.0, false));	
		...
		full_path.push_back(TankDriveCalculator (SplineWrap (
						...
						{ mid_point, 0.25, 1.0, MiscMath::pi / 2.0f, wheel_width, true }
						), 660.0 / 2.0, 20.0, true));	

		full_path.push_back(TankDriveCalculator (SplineWrap (
						{ mid_point, 0.25, 1.0, MiscMath::pi / 2.0f, wheel_width, false },
						{ FieldDimension::Scale::front_center_left - cv::Point2f(0.0, wheel_width / 2.0f), 0.25, 1.0, MiscMath::pi / 2.0f, wheel_width, false }

...
namespace FieldDimension {
	...
	const float plate_width = IN_TO_MM(36.0);
	const float plate_height = IN_TO_MM(48.0);

	const float field_width = IN_TO_MM(314.0);
	const float field_height = IN_TO_MM(400.0);

	const cv::Rect2f field_bounds (0, 0, field_width, field_height);	

	namespace Switch {
		const float boom_width = IN_TO_MM(72.0);
		const float boom_height = IN_TO_MM(12.0);
		const float plate_vertical_offset = IN_TO_MM(140.0);
		const float plate_horizontal_offset = IN_TO_MM(85.25);
	...
	namespace Scale {
		const float boom_width = IN_TO_MM(104.0);
		const float boom_height = IN_TO_MM(12.0);
		const float plate_vertical_offset = IN_TO_MM(299.65);
		const float plate_horizontal_offset = IN_TO_MM(71.57);

...
class MultiWaypointCalculator {
	public:
		...
		void push_back(WayPoint input, bool reverse, RioCommand::Action action);
		bool evaluate(TankDriveCalculator::TankOutput &output, RioCommand::Action &action);
	...
	private:
		...
		WayPoint beginning;
		RioCommand::Action beginning_action;
